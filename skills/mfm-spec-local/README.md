...
responsibilities, features with acceptance, criteria that actively pressure the design,
and lightweight evaluations that record feedback before it evaporates.
...
- **Captures the *why*, not just the *what*.** Components record rationale and
  contracts; features record behavior and acceptance; criteria record cross-cutting
  guidance and assessment; evaluations record judgment and evidence.
...
`features/`; criterion files live under `criteria/`; evaluation files live under
`evaluations/`. Each carries a
...
versioned, agent-native specification format. The current version is **v0.5** —
components, features, criteria, evaluation notes, and provenance-preserving node
supersession. The normative definition — fields, body contract, integrity rules, and a
Pydantic reference schema — is in
