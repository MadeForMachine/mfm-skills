...
validated mutations through MCP, and keeps revision history in the service database. The
customer's own agent performs the authoring and semantic judgment; the service does not run
model inference and keeps persistence, projection, validation, and admission deterministic.
...
Each hosted session starts by reading the project's authoritative policy. That policy governs
the agent guidance, deterministic constraints, and available tool profile; portable criteria
remain ordinary nodes inside the MFM Spec artifact.
